...
public interface CourseRepo extends JpaRepository<CourseEntity,Integer> {
    ...
    @Query(nativeQuery = true,value = "SELECT c.id, c.course_id,c.course_name,c.hours,c.type,c.credit,c.department_id,c.level,c.semester FROM course c INNER JOIN coursecoordinator cc ON c.course_id = cc.course_id inner join mark_approved_level on c.course_id = mark_approved_level.course_id WHERE cc.user_id = (SELECT user_id FROM user WHERE email =:email) and mark_approved_level.approval_level = 'finalized'")
    List<CourseEntity> findLecturerApprovedCourses(@Param("email") String email);

    @Query(nativeQuery = true,value = "select c.id, c.course_id,c.course_name,c.hours,c.type,c.credit,c.department_id,c.level,c.semester from course c inner join coursecoordinator cc on  c.course_id = cc.course_id where cc.user_id =(select user_id from user where email =:email)")

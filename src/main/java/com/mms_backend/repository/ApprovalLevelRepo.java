...
public interface ApprovalLevelRepo extends JpaRepository<MarksApprovalLevel,Integer>
{
    ...
    @Modifying
    @Query(nativeQuery = true,value ="UPDATE mark_approved_level SET approval_level =:approval_level WHERE course_id=:course_id and academic_year=:academic_year and department_id=:department_id")
    void updateApprovedLevel(@Param("course_id") String course_id, @Param("academic_year")String academic_year, @Param("approval_level") String approval_level,@Param("department_id") String department_id);
    ...
    @Query(nativeQuery = true,value = "select * from mark_approved_level where course_id=:course_id and  department_id=:department_id")

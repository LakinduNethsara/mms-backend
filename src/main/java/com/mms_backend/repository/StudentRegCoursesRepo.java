...
public interface StudentRegCoursesRepo extends JpaRepository<StudentRegCourses,String>
{
    ...
    @Query(value = "SELECT * FROM StudentRegCourses WHERE course_id =:course_id AND StudentRegCourses.repeat =:repeat", nativeQuery = true)
    List<StudentRegCourses> getStudentsbyCourseCodeandRepeat(@Param("course_id") String course_id, @Param("repeat") int repeat);
